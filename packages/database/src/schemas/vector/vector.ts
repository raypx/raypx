...
export const vectorEmbeddings = createTable(
  "embeddings",
  {
    ...
    metadata: jsonb("metadata"), // Additional metadata (document ID, dataset ID, etc.)
 ...
 * Otherwise, chunks are stored in the main database (rag.chunks table).
 */
export const vectorChunks = createTable(
  "chunks",
  {
    ...
    index: text("index").notNull(), // Chunk index within document
    documentId: uuid("document_id"), // Reference to document (may be in different database)
    datasetId: uuid("dataset_id"), // Reference to dataset (may be in different database)
  ...
  (table) => [
    index("idx_vector_chunks_document_id").on(table.documentId),
    index("idx_vector_chunks_dataset_id").on(table.datasetId),
    index("idx_vector_chunks_user_id").on(table.userId),
    index("idx_vector_chunks_user_id_document_id").on(table.userId, table.documentId),
  ],
);
