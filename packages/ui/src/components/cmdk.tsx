...
import { commandScore } from "../lib/cmdk";
...
const Item = React.forwardRef<HTMLDivElement, ItemProps>((props, forwardedRef) => {
  ...
  useLayoutEffect(() => {
    if (!forceMount) {
      return context.item(id as string, groupContext?.id ?? "");
    }
  }, [forceMount]);
  ...
  const render = useCmdk((state) =>
    forceMount
      ...
      : context.filter() === false
        ...
        : !state.search
          ? true
          : (state.filtered.items.get(id as string) ?? 0) > 0,
  );
  ...
  const render = useCmdk((state) =>
    forceMount
      ...
      : context.filter() === false
        ...
        : !state.search
          ? true
          : state.filtered.groups.has(id as string),
  );

  useLayoutEffect(() => {
    return context.group(id as string);
  }, []);
    ...
    >
      ...
      {SlottableWithNestedChildren(props, (child) => (
        <div aria-labelledby={heading ? headingId : undefined} cmdk-group-items="" role="group">
          <GroupContext.Provider value={contextValue as Group}>{child}</GroupContext.Provider>
        </div>
      ))}
    </div>
  );
...
function useValue(
  ...
  useLayoutEffect(() => {
    ...
    if (value) {
      context.value(id as string, value, keywords);
