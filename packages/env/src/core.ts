...
/**
 * Optimized merge using UnionToIntersection - simpler and more efficient
 * @internal
 */
type MergeAllOptimized<T extends readonly Record<string, any>[]> = UnionToIntersection<T[number]>;
 ...
 * Common examples of prefixes are `VITE_`, `NUXT_PUBLIC` or `PUBLIC_`.
 */
...
>(
  ...
  const parsed =
    opts
      ...
      ["~standard"].validate(runtimeEnv) ?? parseWithDictionary(finalSchemaShape, runtimeEnv);
