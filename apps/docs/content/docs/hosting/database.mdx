...
- name: Run migrations
  ...
  run: pnpm --filter @raypx/database run db:migrate
```
...
- **Drizzle Studio**: `pnpm --filter @raypx/database run db:studio`
...
- Check migration status: `pnpm --filter @raypx/database run db:migrate`
...
- Review [Development Workflow](/docs/development) for database development practices
- Check [Workspace Layout](/docs/workspace) for database package structure
- Set up [Redis hosting](/docs/redis-hosting) for caching (optional)
