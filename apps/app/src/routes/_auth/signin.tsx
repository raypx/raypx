import { createSignInFormSchema, MagicLinkButton, useAuth, useSignIn } from "@raypx/auth";
import {
  Button,
  Checkbox,
  Field,
    ...
    >
      ...
      >
        ...
        <Button className="w-full" disabled={form.state.isSubmitting} type="submit">
          {form.state.isSubmitting ? <IconLoader className="animate-spin" /> : "Sign In"}
        </Button>

        <MagicLinkButton currentView="sign-in" disabled={form.state.isSubmitting} />
      </form>
    </AuthCard>
  );
}
